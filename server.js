...
/*
START: New function createComment
*/
...
function createComment(url, request) {
    ...
    if (requestComment && requestComment.body && requestComment.articleId && database.articles[requestComment.articleId] &&
        ...
        console.log('#requestComment body: ' + requestComment.body + ' ,url: ' + url);
        console.log('#RequestComment body comment: ' + requestComment.body.comment + ' ,url: ' + url);
        console.log("Username: " + requestComment.username);
        console.log("RequestComment.articleId: " + requestComment.articleId);
    }
...
/*
END: New function createComment
*/
...
function deleteComment(url, request) {
    ...
    if (savedComment) {
        ...
        savedComment.articleIds.forEach(articleId => {
            ...
            database.comments[articleId] = null;
            const userCommentIds = database.users[comment.username].commentIds;
            userCommentIds.splice(userCommentIds.indexOf(id), 1);
        });
        */
...
function loadDatabase() {
    try {
        let doc = yaml.safeLoad(fs.readFileSync('database.yml', 'utf8'));
        console.log(doc);
    } catch (e) {
        console.log(e);
    }
...
function saveDatabase() {
    ...
    if (doc) {
        fs.writeFileSync("database.xml", doc);
        return true;
    } else {
        return false;
    }
